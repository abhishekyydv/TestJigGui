...
if (window.api && window.api.onData) {
  window.api.onData((data) => {
    ...
    safeUpdate(
      "chargingStatus",
      efficiency > 90 ? "PASS" : "FAIL",
      "passfail-bar " + (efficiency > 90 ? "PASS" : "FAIL")
    );

    // Discharging section
    const disInputPower = arr[4] * arr[5];
    const disOutputPower = arr[6] * arr[7];
    const disEfficiency = disInputPower
      ? (disOutputPower / disInputPower) * 100
      : 0;
    ...
    safeUpdate(
      "dischargingStatus",
      disEfficiency > 90 ? "PASS" : "FAIL",
      "passfail-bar " + (disEfficiency > 90 ? "PASS" : "FAIL")
    );
  });
}
